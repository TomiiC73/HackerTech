...
const CAREERS = [
  { name: "Ingeniería en Sistemas de Información", meta: "Grado · 5 años" },
  { name: "Ingeniería Civil", meta: "Grado · 5 años" },
  { name: "Ingeniería Electrónica", meta: "Grado · 5 años" },
  { name: "Tecnicatura en Mecatrónica", meta: "Tecnicatura · 2 años" },
];
...
export default function App() {
  ...
  const [home, setHome] = useState({ news_items: [], quick_access_items: [], domain_options: [], default_domain: "frc" });
        ...
        <section id="carreras" className="py-20 sm:py-28 px-5 sm:px-8 bg-white dark:bg-carbon-900" aria-labelledby="carreras-title">
          <div className="max-w-7xl mx-auto">
            ...
            <h2 id="carreras-title" className="text-2xl sm:text-3xl font-extrabold text-navy-900 dark:text-white mb-10">
              Carreras de grado y tecnicaturas
            </h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {CAREERS.map((career) => (
                <div
                  key={career.name}
                  className="border border-slate-200 dark:border-carbon-700 rounded-2xl p-5 transition-all hover:scale-[1.02] hover:shadow-premium hover:border-transparent"
                >
                  <div className="font-bold text-navy-900 dark:text-white mb-1">{career.name}</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">{career.meta}</div>
                </div>
              ))}
            </div>
          </div>
