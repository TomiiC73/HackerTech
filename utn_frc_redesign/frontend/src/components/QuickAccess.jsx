        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item, index) => (
            <Reveal key={item.label} delay={index * 0.06}>
              <a
                href="#login"
                className="group bg-white dark:bg-carbon-900 border border-slate-200 dark:border-carbon-700 rounded-2xl p-6 flex flex-col gap-2 shadow-sm transition-all hover:-translate-y-1 hover:shadow-premium hover:border-transparent focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
                <span
